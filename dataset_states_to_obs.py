...
def get_observations_from_states(state):
    ret = {}
    ret["pos"] = np.array(state[:2]) # fetch position indexes from state array
    ret["vel"] = np.array(state[2:]) # fetch velocity indexes from state array
    # ret["ctrl"] = np.array(state[3]) # Control is an action, for cartpole, each state is only position and velocity 
    return ret
...
def extract_trajectory(
    ...
    for t in range(1, traj_len + 1):
        ...
        if t == traj_len:
            ...
            next_obs = get_observations_from_states(states[t-1]) #KLUDGE (woooow, Shin)
        else:
